...
case "$KIND" in
  traefik)
    ...
    info "network: $NET"
    CID="$(docker compose -f "$HERE/docker-compose.yml" ps -q whatsapp-service)"
    docker network connect "$NET" "$CID" 2>/dev/null || info "already attached"
    warn "Traefik reads its routes from container labels, which cannot be added to a"
    warn "running container. Add these to deploy/docker-compose.yml under the service,"
...
EOF
    warn "The certresolver name must match the one n8n's own labels use. Check with:"
    warn "    docker inspect $PROXY_NAME -f '{{json .Config.Labels}}' | tr ',' '\\n' | grep -i certresolver"
    ;;
...
bold "Next"
# ---------------------------------------------------------------------------
info "1. Watch it come up:   docker compose -f $HERE/docker-compose.yml logs -f"
