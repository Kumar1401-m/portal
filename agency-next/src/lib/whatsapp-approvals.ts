...
export async function recordApproval(input: ApprovalInput): Promise<ApprovalResult> {
  ...
  if (!videoCode) {
     ...
     *
     * Written content used to be checked first: a client sent the month's copy
     * and replying "ok" was answering that rather than a video from last week.
     * Content no longer goes to a client at all — it is settled inside the
     * agency — so an "ok" in a group can only be about a video, and reading it
     * as anything else would move a task nobody was asked about.
     */
